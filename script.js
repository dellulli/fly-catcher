...
let score = 0;
const totalFlies = 60; // Updated goal to catch 60 flies
const timeLimit = 16; // seconds
const flyInterval = 300; // Interval for faster fly spawning
const flyLifetime = 2000; // Lifetime of each fly (in milliseconds)
const specialItemInterval = 1000; // Faster interval for adding special items (in milliseconds)
...
function addFly() {
    ...
    fly.style.top = `${Math.random() * (gameArea.clientHeight - 50)}px`; // Adjust for fly height
    fly.style.left = `${Math.random() * (gameArea.clientWidth - 50)}px`; // Adjust for fly width
...
function addSpecialItem() {
    ...
    specialItem.style.top = `${Math.random() * (gameArea.clientHeight - 50)}px`; // Adjust for item height
    specialItem.style.left = `${Math.random() * (gameArea.clientWidth - 50)}px`; // Adjust for item width
...
function endGame(won, specialItemSrc) {
    ...
    if (won) {
        ...
        if (specialItemSrc && specialItemSrc.includes('vapeleak.png')) {
            message.textContent = 'Game Over! You caught a vape leak';
        } else if (specialItemSrc && specialItemSrc.includes('warmwater.png')) {
            message.textContent = 'Game Over! You caught warm water';
        } else {
